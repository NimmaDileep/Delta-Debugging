    ...
    while len(inp) >= 2:
        ...
        while start < len(inp):
            ...
            if test(complement, *test_args) == Result.FAIL:
                ...
                data = inp
                occurrences = re.finditer("&#x[0-9a-f]+", data)
                # assuming only one occurrence
                occurrence = next(occurrences)
                start, end = occurrence.start(), occurrence.end()
                start_tags, end_tags = (
                    tuple(
                        re.finditer(
                            r"<[a-zA-Z-_ ]+( ([a-zA-Z0-9-_]+(=[a-zA-Z0-9-_\"']+)?))?>", data[:start]
                        )
                    ),
                    tuple(re.finditer(r"</[a-zA-Z-_ ]+>", data[end + 1:])),
                )
...
with open("xmlproc/demo/urls.xml") as f:
    ...
    print('The reduced input for which the failure occurs :')
